...
def irc2xyz(coord_irc, origin_xyz, vxSize_xyz, direction_a):
    cri_a = np.array(coord_irc)[::-1] #反向打印数据，从最后一个index开始，间隔为1。cri_a是一个tuple(c,r,i)
    origin_a = np.array(origin_xyz) #原点偏移量
    vxSize_a = np.array(vxSize_xyz) #两个像素间的距离
...
def xyz2irc(coord_xyz, origin_xyz, vxSize_xyz, direction_a):
    ...
    cri_a = ((coord_a - origin_a) @ np.linalg.inv(direction_a)) / vxSize_a  #np.linalg.inv矩阵求逆

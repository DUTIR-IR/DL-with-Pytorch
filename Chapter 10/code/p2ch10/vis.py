...
def showCandidate(series_uid, batch_ndx=None, **kwargs):#显示某个uid对应的ct图像
    ...
    '''绘制原始的ct图（2d）index、row、column三个维度的值分别取成相应的结节中心坐标'''
    subplot = fig.add_subplot(len(group_list) + 2, 3, 1)#在图上画子图，fig.add_subplot(5,3,1)
    subplot.set_title('index {}'.format(int(center_irc[0])), fontsize=30)
    for label in (subplot.get_xticklabels() + subplot.get_yticklabels()):
        label.set_fontsize(20)       
    #imshow()将numpy数组绘制为图像
    plt.imshow(ct.hu_a[int(center_irc[0])], clim=clim, cmap='gray')#center_irc[0]是index,即从3d立体ct图中得到一个2d的图像切片
